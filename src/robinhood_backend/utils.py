...
def l2_chart(symbol:str, min_price:float|None=None, max_price:float|None=None):
    ...
    min_price = min_price or (current_price * .5)
    max_price = max_price or (current_price * 1.5)

    # Filter the data to the specified range
    ask_orders_filtered = ask_orders[(ask_orders['price'] >= min_price) & (ask_orders['price'] <= max_price)]
    bid_orders_filtered = bid_orders[(bid_orders['price'] >= min_price) & (bid_orders['price'] <= max_price)]
    ...
    fig.add_shape(
        type="line",
        x0=current_price, y0=0, x1=current_price, y1=1,
        yref="paper",
